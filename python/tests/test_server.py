...
@pytest.fixture
def mock_generators():
    with patch('server.sysinfo_generator') as mock_sysinfo_generator, \
            patch('server.delete_model_handler') as mock_delete_model_handler, \
            patch('server.install_generator') as mock_install_generator, \
            patch('server.run_models_generator') as mock_run_models_generator, \
            patch('server.stop_model_handler') as mock_stop_model_handler, \
            patch('server.get_highlights') as mock_get_highlights, \
            patch('server.get_new') as mock_get_new:
        yield (mock_sysinfo_generator, mock_delete_model_handler, mock_install_generator, mock_run_models_generator, mock_stop_model_handler, mock_get_highlights, mock_get_new)
...
def test_install_model(mock_generators):
    ...
    response = client.post(
        "/model/install", json={"id": "aaaa-bbbb-cccc-dddd", "url": "https://example.com/model.zip"})
    assert response.status_code == 200
    assert response.headers["Cache-Control"] == "no-cache"
    assert response.headers["Connection"] == "keep-alive"
