...
import {
  isConvertableWeightFormat,
  isTruffleFormat,
  truffleChatConfigExistsAndIsValid,
  convertModelWeights,
  genChatConfig,
...
export class ModelManager {
  ...
  async getRepoInfo(hfRepoId: string): Promise<any> {
    ...
    const res = await axios.get(
      `https://huggingface.co/api/models/${hfRepoId}?`
    );
  ...
  async downloadModel(hfRepoId: string) {
    ...
    for (const file of files) {
      ...
      response.data.on("data", (chunk: any) => {
        ...
        if (
          (100 * bytesDownloaded) / totalSize - this.downloads[uuid].progress >=
          0.25
        ) {
          this.downloads[uuid].progress =
            0.25 * Math.floor((100 * bytesDownloaded) / totalSize / 0.25);
