...
@pytest.mark.asyncio
async def test_ram_change_detection(mocker):
    ...
    mocker.patch(
        "psutil.virtual_memory", return_value=MagicMock(total=100000000, available=after_change)
    )
...
@pytest.mark.asyncio
async def test_no_significant_ram_change_does_not_yield(mocker):
    # Start the sysinfo generator
    generator = sysinfo_generator()
    initial_data = await generator.__anext__()  # Get initial data
...
@pytest.mark.asyncio
async def test_disk_change_detection(mocker):
    # Start the sysinfo generator
    generator = sysinfo_generator()
    initial_data = await generator.__anext__()  # Get initial data

    initial_disk = json.loads(initial_data.split("data: ")[1].strip())["resources"]["available"][
        "disk"
    ]
    ...
    mocker.patch(
        "psutil.disk_usage",
        return_value=MagicMock(total=500000000, used=after_change, free=500000000 - after_change),
    )
    updated_data = await generator.__anext__()

    # Parse the JSON data from the generator output
    updated_disk = json.loads(updated_data.split("data: ")[1].strip())["resources"]["available"][
        "disk"
    ]
...
@pytest.mark.asyncio
async def test_no_significant_disk_change_does_not_yield(mocker):
    # Start the sysinfo generator
    generator = sysinfo_generator()
    initial_data = await generator.__anext__()  # Get initial data
    ...
    mocker.patch(
        "psutil.disk_usage", return_value=MagicMock(total=1000000, used=1000000, free=after_change)
    )
    try:
        await asyncio.wait_for(generator.__anext__(), timeout=10)
