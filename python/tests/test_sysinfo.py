            ...
            async for line in response.aiter_lines():
                if line == "":
                    ...
                    try:
                        ...
                        pytest.fail(
                            f"JSON data did not validate against schema: {e}")
...
async def test_ram_change_detection():
    ...
    async with init_db():
        ...
        initial_ram = json.loads(initial_data.split("data: ")[1].strip())[
            'resources']['available']['ram']
        updated_ram = json.loads(updated_data.split("data: ")[1].strip())[
            'resources']['available']['ram']
        ...
        assert ram_change >= CHANGE_THRESHOLD, f"RAM change should be at least {
            CHANGE_THRESHOLD}%"
...
async def test_disk_change_detection():
    ...
    async with init_db():
        ...
        # Parse the JSON data from the generator output
        initial_disk = json.loads(initial_data.split("data: ")[1].strip())[
            'resources']['available']['disk']
        updated_disk = json.loads(updated_data.split("data: ")[1].strip())[
            'resources']['available']['disk']

        # Calculate the percentage change in disk usage
        disk_change = abs(updated_disk - initial_disk) / initial_disk * 100

        assert disk_change >= CHANGE_THRESHOLD, f"Disk usage change should be at least {
            CHANGE_THRESHOLD}%"
