...
export function isConvertableWeightFormat(modelPath: string): boolean {
  const pytorch_json_path = path.resolve(
    modelPath,
    "pytorch_model.bin.index.json"
  );
  const pytorch_bin_path = path.resolve(modelPath, "pytorch_model.bin");
  const safetensors_path = path.resolve(
    modelPath,
  ...
  return (
    fs.existsSync(pytorch_json_path) ||
    fs.existsSync(pytorch_bin_path) ||
    fs.existsSync(safetensors_path) ||
    fs.existsSync(safetensors_bin_path)
  );
}
...
export function ndArrayCacheExistsAndIsValid(modelPath: string): boolean {
  ...
  try {
    ...
    existsAndValid =
      baseKeyValidity && recordKeysValidity && isDataPathValid && isFormatValid;
  } catch (error) {
    existsAndValid = false;
  }
...
export async function convertModelWeights(
  modelPath: string,
  systemRAM: number,
  modelSize: number
) {
  ...
  await new Promise((resolve, reject) => {
    res.on("close", (code: any) => {
      if (code === 0) {
        ...
        reject(
          new Error(`Failed to convert model weights. Exit code: ${code}`)
        );
      }
    });
  });
...
export async function genChatConfig(
  modelPath: string,
  systemRAM: number,
  modelSize: number
) {
