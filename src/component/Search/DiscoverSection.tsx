...
import { useGetFeatured } from "../../lib/react-query/queriesAndMutations";
    ...
    <div className="flex justify-center items-center gap-2.5">
      {tabs.map((tab) => (
        ...
        >
          ...
          {/* if tab is New then show one svg */}
        {
          tab === "New" && (
...
const DiscoverGrid: React.FC = () => {
  const { data: featuredModels, isLoading, error } = useGetFeatured();
  ...
  return (
    <div className="w-full p-8 grid grid-cols-4 gap-x-6 gap-y-6">
      {featuredModels?.map((model: TModel) => (
